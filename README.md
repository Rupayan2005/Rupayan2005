<h1 align="center">
  <img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=600&size=28&duration=3500&pause=1000&color=00FF99&center=true&vCenter=true&width=600&height=50&lines=Hi+%F0%9F%91%8B%2C+I'm+Rupayan+Auddya;" alt="Typing SVG" />
</h1>

<h3 align="center">
  <img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=500&size=22&duration=3500&pause=1000&color=00FF00&center=true&vCenter=true&width=650&height=35&lines=A+Passionate+Web+%26+ML+Developer+%F0%9F%94%A5;Transforming+Ideas+into+Intelligent+Solutions+%F0%9F%92%A1;Crafting+Modern+Web+Experiences+%F0%9F%8C%90;Building+the+Future+with+AI+%F0%9F%92%BB" alt="Typing SVG" />
</br>
</br>
</h3>



## 🌟 Meet the Mind Behind the Code

A curious technologist on a mission to turn imagination into innovation—exploring the cutting edge of AI and crafting the future with modern web tech.

```python
class Developer:
    def __init__(self):
        self.name = "Rupayan Auddya"
        self.role = "AI Enthusiast & Full-Stack Developer"
        self.language_spoken = ["Python", "JavaScript", "C", "C++", "English", "Bengali", "Hindi", "Logic"]
        self.current_focus = "Building intelligent systems"
    
    def say_hi(self):
        print("Thanks for dropping by! Let's build something amazing together 🚀")

me = Developer()
me.say_hi()
```

## 🧠 My Tech Universe

### 🤖 AI & Machine Learning
- **Machine Learning**: Turning data into insights
- **Computer Vision**: Teaching machines to see the world
- **Natural Language Processing**: Bridging human-computer communication
- **Generative AI & LLMs**: Crafting the next generation of intelligent systems

### 🌐 Web Development
- Frontend magic with modern frameworks
- Backend architecture that scales
- Full-stack solutions from concept to deployment

### 🔧 Problem Solving
- **Data Structures & Algorithms**: The foundation of efficient code
- **System Design**: Building robust, scalable applications


<h3 align="left">🧑‍💻⚙️ Languages & Tools</h3>
</br>
<a href="https://skillicons.dev">
    <img src="https://skillicons.dev/icons?i=c,cpp,py,r,js,react,nextjs,ts,vite,tailwind,html,css,nginx,nodejs,npm,pnpm,docker,fastapi,git,github,mongodb,postgres,vscode,sklearn,tensorflow,opencv" />    
</a>

## 🎯 Current Mission

```
[████████████████████████████████████████] 100% Loading awesome projects...
```

🔍 **Now Exploring**: The intersection of AI and human creativity  
🌱 **Growing**: Expanding my knowledge in cutting-edge ML techniques  
🚀 **Building**: Projects that make a difference  

## 🎲 When I'm Not Coding

- ♟️ **Chess Enthusiast**: Strategic thinking on and off the board
- 🎬 **Movie Buff**: Analyzing stories and cinematography
- 📚 **Perpetual Learner**: Always adapting to new concepts and technologies
- 🧩 **Problem Solver**: Finding elegant solutions to complex challenges


## 🎨 Portfolio

✨ Ready to see creativity meet code? Dive into my digital showcase below!

<div align="center">
</br>

[![🚀 Explore Portfolio](https://img.shields.io/badge/🚀_Explore_Portfolio-6C63FF?style=for-the-badge&logo=vercel&logoColor=white)](https://portfolio-37z9.vercel.app/)

</div>


## 📊 GitHub Analytics

<div align="center">

<!-- GitHub Stats -->
<a href="https://github.com/Rupayan2005/github-readme-stats">
  <img width="48%" src="https://github-readme-stats.vercel.app/api?username=Rupayan2005&show_icons=true&theme=highcontrast&locale=en" alt="GitHub Stats" />
</a>

<!-- Top Languages -->
<a href="https://github.com/Rupayan2005/github-readme-stats">
  <img width="48%" src="https://github-readme-stats.vercel.app/api/top-langs/?username=Rupayan2005&layout=compact&theme=highcontrast&locale=en" alt="Top Languages" />
</a>

<!-- Most Used Languages by Commit -->
<a href="https://github.com/Rupayan2005/github-readme-stats">
  <img width="45%" src="http://github-profile-summary-cards.vercel.app/api/cards/most-commit-language?username=Rupayan2005&theme=highcontrast" alt="Most Used Languages by Commit" />
</a>

<p><img align="center" src="https://github-readme-streak-stats.herokuapp.com/?user=rupayan2005&theme=github_dark" alt="Github Streak" /></p>
</br>
</div>



## 📈 GitHub Activity Timeline

![](https://github-readme-activity-graph.vercel.app/graph?username=rupayan2005&bg_color=0d1117,0d1117,000000&color=00ffcc&line=00ffe4&point=ffff00&area=true&hide_border=true)

## 🎖️ GitHub Wall of Fame
<div align="center">
<img src="https://github-trophies.vercel.app/?username=rupayan2005&theme=radical" alt="Github Trophies" />
</div>

## 🎪 Fun Facts

- 🎯 I solve coding challenges the same way I approach chess - thinking several moves ahead
- 🤔 I debug code like I analyze movies - looking for plot holes and inconsistencies
- 🔄 I believe the best code is like a good movie - it tells a story that's easy to follow
- 🎨 I think AI is the new paintbrush for digital artists



<h3 align="left">📬 Let's Connect!</h3>
<p align="left">
</br>
<a href="https://linkedin.com/in/rupayan auddya" target="blank"><img align="center" src="https://raw.githubusercontent.com/rahuldkjain/github-profile-readme-generator/master/src/images/icons/Social/linked-in-alt.svg" alt="rupayan auddya" height="30" width="40" /></a>
</p>

</br>

<hr>



### 🎨 Random Dev Quote

<p align="center">
  <img src="https://quotes-github-readme.vercel.app/api?type=horizontal&theme=radical" alt="Random Dev Quote" />
</p>

<div align="center">

**🚀 Ready to build something extraordinary together?**

*Let's turn ideas into reality!*

![Visitor Count](https://komarev.com/ghpvc/?username=rupayan2005&color=blueviolet&style=flat-square)

</div>

---
